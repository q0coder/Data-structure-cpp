...
private:
	...
	bool isBalance(Node* node,int l, bool& flag)
	{
		if (node == nullptr)
		{
			return l;
		...
		int left = isBalance(node->left, l + 1, flag);
		if (!flag)
			return left;
		int right = isBalance(node->right, l + 1, flag);
		if (!flag)
			return right;
	...
	Node* getVal(Node* node, int k,int &num)
	{
		if (node == nullptr)
		{
			return nullptr;
		}
		Node* left = getVal(node->right, k,num);
		if (left != nullptr)
			return left;
		num++;
		if (num == k)
...
int main()
{
	...
	//cout << bst1.getVal(2) << endl;
	cout<<bst1.isBalance()<<endl;
	//bst1.inOrder();
	bst1.n_insert(12);
	//bst1.inOrder();
